...
constant cvs_version="$Id: roxen.pike,v 1.580 2001/11/01 15:57:09 grubba Exp $";
...
void unregister_url( string url )
{
  ...
  // No need to unregister this... It cannot exist
...
#ifdef THREADS
      object mutex_key, threads_disabled;
      if (!from_handler_thread) {
...
int main(int argc, array tmp)
{
  ...
  // Dangerous...
  mixed tmp2;
  if(tmp2 = Getopt.find_option(argv, "r", "root")) fix_root(tmp2);
...
({
  ...
  ({ "host",        "\4711",    0, 1 }), // \4711 is unlikely to occur normally
});
