...
class ChiliMoonSessionContext 
{
  ...
  public synchronized HttpSession getSession(String id, boolean create)
  {
	...
	ChiliMoonSession session = (ChiliMoonSession) sessions.get(garbID);
    ...
    if(id != null) {
      ...
      if(s != null) {
	ChiliMoonSession session = (ChiliMoonSession)s;
	if(session.access())
	  return session;
	else {
	  sessions.remove(id);
    ...
    if(!create)
      return null;
    ChiliMoonSession session = new ChiliMoonSession();
