...
//! Set to 1 when this request object has been turned into a websocket
int is_websocket = 0;

//! The @[Protocols.WebSocket.Connection] object when we are turned
//! into a websocket.
object websocket;
...
protected void ws_handle_queue() {
  ...
  mixed msg = ws_msg_queue->read();
  if (objectp(msg)) {
    // This is a WebSocket frame so let's process it.
    conf->websocket_handle(this, msg, "websocket_message");
  } else if (msg == WS_OPEN_MSG) {
    // This is the first message so let's call the open callback
    conf->websocket_handle(this, msg, "websocket_ready");
  } else if (msg == WS_CLOSE_MSG) {
    conf->websocket_handle(this, ws_close_reason, "websocket_close");
    end();
    return;
  }
...
protected void upgrade_to_websocket(int masking,
  ...
  while(sizeof(data)) {
    int res = data->output_to(fd);
    if (res == -1) {
      werror("Failed to send data: %s\n", strerror(errno()));
      end();
      return;
    }
  }
...
#define LOG_HANDLE_END()
  ...
  if(e)
    ...
    if (result && result->upgrade_websocket) {
      ...
      upgrade_to_websocket(result->masking, result->extra_headers || ([]));
