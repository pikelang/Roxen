...
1;}void t2(string name){sscanf(name,"%[a-zA-Z0-9едц≈ƒ÷ь№]s\n",name);if(!strlen
(name)){io->get_name(t2);}else{tm=name;io->raw();io->set_read_callback(k);
call_out(tick,h);for(;++_<277;)q[276-_]=(_<25||_%12<2)&&7;io->write("\33[H\33"
"[J");u();}}void tetris(object input){a=(array)"jkl pq";h=0.5;io=input;input->
get_name(t2);}class telnet{inherit Stdio.File;mixed д;void do_get_name(mixed _
,string name){д(name);}string get_name(mixed w){д=w;write("What is your name?"
" ");set_read_callback(do_get_name);}void create(object f){assign(f);destruct(
f);}void raw(){write("€э\"€ъ\"\1\0р€ы\1€э\3€ь\3");}void normal(){write("€э\1€"
"э\"");}void quit(){destruct(this_object());}}void create(object f){if(f)
tetris(telnet(f));}class console{inherit Stdio.File;string get_name(mixed cb){
cb(getenv("LOGNAME")||popen("id -un"));}void create(){::create("stdin");}void
raw(){system("stty raw cbreak -echo stop u");}void normal(){system("stty sane"
);}void quit(){exit(0);}}int main(){tetris(console());return -1;}
