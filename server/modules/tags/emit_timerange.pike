...
constant cvs_version = "$Id: emit_timerange.pike,v 1.30 2008/12/22 15:13:20 mast Exp $";
...
static mapping(string:Calendar) cached_calendars = ([]);

Calendar get_calendar(string name)
{
  if(!name)
    return calendar;
  if (Calendar cal = cached_calendars[name])
    return cal;
...
static RoxenModule rxml_sql_module;
