...
string cvs_version = "$Id: ftpgateway.pike,v 1.48 2004/06/05 15:19:47 _cvs_dirix Exp $";
#include <module.h>
#include <config.h>
...
\r\n
<title>ChiliMoon internal error</title>\n
...
string INFOSTRING="<font size=\"-2\"><a href=\"http://www.roxen.com/\">"+core->version()+
                  "</a> FTP Gateway "+("$Revision: 1.48 $"-"$")+"</font>";
...
#define set_what_now(s) (what_now=(s))
			...
			void|string type,void|string icon, void|string desc)
  {
	...
	return sprintf("<a href=\"%s\"><imgs src=%s border=0 alt=\"\" /> %-*s"
		       "</a>%12s %s %s\n<a href=\"%s\"><imgs src=%s /></a>\n",
    ...
    if (master->query("icons")=="Yes"&& effect!="noicons")
      return sprintf("<a href=\"%s\"><imgs src=%s border=0 alt=\"\" /> %-*s</a>"
		     "%12s %s %s \n",
