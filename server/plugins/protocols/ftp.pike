	   ...
	   static int|void flags)
{
  ...
  string ls_l(string file, Stdio.Stat st)
  {
    DWRITE("ls_l(\"%s\")\n", file);
    ...
    mapping lt = localtime(st->mtime);

    // NOTE: SiteBuilder may set st[5] and st[6] to strings.
    string user = (string)st->uid;
    string group = (string)st->gid;
    if (!(flags & LS_FLAG_n)) {
    ...
    if ((st->mtime <= now - 15778800) || (st->mtime > now)) {
      // Month Day  Year
...
class FTPSession
{
  ...
  string make_MLSD_fact(string f, mapping(string:array) dir, object session)
  {
    ...
    facts->modify = make_MDTM(st->mtime);
  ...
  void create(object fd, object c)
  {
    ...
    s = replace(s,
		({ "$roxen_version", "$roxen_build", "$full_version",
		   "$pike_version", "$ident", }),
		({ core.__chilimoon_version__, core.__chilimoon_build__,
