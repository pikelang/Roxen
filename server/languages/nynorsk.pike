...
// Norwegian Nynorsk language plugin

inherit "abstract.pike";

constant cvs_version = "$Id$";
constant _id = ({ "nn", "nynorsk", "" });
constant _aliases = ({ "nno", "nynorsk" });
...
string date(int timestamp, mapping m)
{
  ...
  if(!(m["full"] || m["date"] || m["time"]))
  {
    if(t1["yday"] == t2["yday"] && t1["year"] == t2["year"])
      return "i dag, klokka " + ctime(timestamp)[11..15];

    if(t1["yday"] == t2["yday"]-1 && t1["year"] == t2["year"])
      return "i går, klokka " + ctime(timestamp)[11..15];

    if(t1["yday"] == t2["yday"]+1 && t1["year"] == t2["year"])
      return "i morgon, ved "  + ctime(timestamp)[11..15];
