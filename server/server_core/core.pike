...
constant cvs_version="$Id: core.pike,v 1.878 2006/04/19 22:29:56 _cvs_dirix Exp $";
...
#define HAVE_EFFECTIVE_USER
  ...
  void create(string reason, int|string|void uid, int|string|void gid)
  {
    ...
    if(LOGP)
      report_notice("Change to %s(%d):%d privs wanted (%s), from %s",
    ...
    if (err < 0) {
      report_warning("Privs: WARNING: Failed to set the "
  ...
  void destroy()
  {
	...
	array bt = backtrace();
	if (sizeof(bt) >= 2) {
	  report_notice("Change back to uid#%d gid#%d, from %s\n",
			saved_uid, saved_gid, dbt(bt[-2]));
	} else {
	  report_notice("Change back to uid#%d gid#%d, from backend\n",
			saved_uid, saved_gid);
	}
      };
    }
...
//! Base protocol for SSL ports. Exactly like Port, but uses SSL.
{
  ...
  void certificates_changed(Variable|void ignored)
  {
    ...
    foreach(map(Certificates->query(), String.trim_whites), string cert_file) {
      ...
      {
        Certificates->add_warning("SSL3: Reading cert-file '%s' failed!",
                                          cert_file); 
        continue;
      }
      ...
      {
        Certificates->add_warning("SSL3: No certificate found.\n");
        continue;
      }
      certificates += ({ cert });
    }
    ...
    {
      KeyFile->add_warning(sprintf("SSL3: Reading key-file '%s' failed!\n",
    ...
    if (!raw_keydata) {
      Certificates->add_warning("SSL3: No private key found.");
      return;
    }
    ...
    SSL3_WERR("key file contains: %O", indices(msg->parts));
    ...
    if (part = msg->parts["RSA PRIVATE KEY"])
    {
      ...
      {
        KeyFile->add_warning("SSL3: Private rsa key not valid (PEM).\n");
        return;
      }
      ...
      if (!rsa)
      {
        KeyFile->add_warning("SSL3: Private rsa key not valid (DER).\n");
        return;
      }

      ctx->rsa = rsa;

      SSL3_WERR("RSA key size: %d bits", rsa->rsa_size());
      ...
      array(int) key_matches =
        map(certificates,
            lambda(string cert, Variable Certificates) {
              ...
              if (!tbs)
              {
                Certificates->add_warning("SSL3: Certificate not valid (DER).");
                return 0;
              }
      ...
      if( !num_key_matches )
      {
        KeyFile->add_warning("SSL3: Certificate and private key "
                                   "do not match.");
        return;
      }
    ...
    {
      KeyFile->add_warning("SSL3: No private key found.");
      return;
    }
...
int register_url( string url, Configuration conf )
{
  ...
  if( (int)opts->nobind )
  {
    report_warning("Not binding the port %O, disabled in configuration\n", url );
    return 0;
  }
  url = replace( url, "/ANY", "/*" );
  url = replace( url, "/any", "/*" );
  ...
  foreach(required_hosts, string required_host)
  {
    ...
    if( !( m[ required_host ][ port ] ) )
    {
      ...
      failures++;
      if (required_host) {
	report_warning("Binding the port on IP %s failed\n   for URL %s!\n",
		       required_host, url);
      }
      continue;
    }
...
//! correct permissions (the ones the owner of that script have).
{
  ...
  if(sizeof(u))
  {
	...
	report_error("It is possible to change uid and gid only "
...
#  if constant(setgid)
	  setgid(gid);
	  if (getgid() != gid) {
	    report_error("Failed to set gid.\n");
	    g = 0;
	  }
#  else
	  if (!from_handler_thread)
...
void reload_all_configurations()
{
  ...
  foreach(list_all_configurations(), string config)
  {
	...
	report_error("Error while enabling configuration %s%s",
		     config, (bt ? ":\n"+bt : "\n"));
	continue;
      }
...
void enable_configurations()
{
  ...
  foreach(list_all_configurations(), string config)
  {
    ...
    if(err=catch( enable_configuration(config)->start(0) ))
      report_error("\nError while loading configuration %s%s",
                   config+":\n", describe_backtrace(err)+"\n");
...
void enable_configurations_modules()
{
  ...
  foreach(configurations, Configuration config)
    if(mixed err=catch( config->enable_all_modules() ))
      report_error("Error while loading modules in configuration %s%s",
                   config->name+":\n", describe_backtrace(err)+"\n");
}
