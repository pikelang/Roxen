...
//
// $Id: module.pmod,v 1.274 2002/11/12 18:32:47 mast Exp $
...
#define EVAL_LOOP(RESOLVE_ARGFUNC_1, RESOLVE_ARGFUNC_2)			\
	...
	chained_p_code_add: {						\
		...
		if (flags & COLLECT_RESULTS &&				\
		  ...
		  if (new_p_code)					\
		    new_p_code->add_frame (ctx, frame, item);		\
		}							\
		else {							\
		  ...
		  if (new_p_code)					\
		    ...
		    if (!exec[pos + 1]) {				\
		      /* Race below, but it doesn't matter much. */	\
		      RESET_FRAME (frame);				\
		      exec[pos + 1] = frame;				\
		    }							\
		}							\
	...
	if (objectp (err) && ([object] err)->thrown_at_unwind) {
	  ...
	  if (new_p_code)
	    if (objectp (item) && item->is_RXML_p_code_frame)
	      new_p_code->add_frame (ctx, frame, PCode);
	    else
