/* $Id: RoxenSSLFile.pike,v 1.24 2007/03/09 21:15:39 mast Exp $
 */
...
class SSLConnection
{
  ...
  // Need to override create() in the old SSL.connection to initialize
  // the context before SSL.handshake.create.
  void create (int is_server, SSL.context ctx)
  {
    context = ctx;
    ::create (is_server);
  }
...
string _sprintf(int t) {
  return t=='O' && sprintf("RoxenSSLFile(%O)", stream && stream->_fd);
}
