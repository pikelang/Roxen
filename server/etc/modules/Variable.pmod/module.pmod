...
//! HTML5-based color picker
{
  ...
  string render_form(RequestID id, void|mapping additional_args)
  {
    ...
    return
      ...
      "  let colEl = document.getElementById('" + color_id + "');\n"
      "  if (colEl) colEl.addEventListener('input', color_sel_cb);\n"
      "  let txtEl = document.getElementById('" + text_id + "');\n"
      "  if (txtEl) {"
      "    txtEl.setAttribute('data-color-id', '" + color_id + "');\n"
      "    txtEl.addEventListener('input', text_inp_cb);\n"
      "  }"
      "  function color_sel_cb(evt) {\n"
      "    let text_id = evt.target.getAttribute('data-text-id');\n"
      "    let txtEl = document.getElementById(text_id);\n"
      "    if (txtEl) txtEl.value = evt.target.value;\n"
      "  }"
      "  function text_inp_cb(evt) {\n"
      "    let color_id = evt.target.getAttribute('data-color-id');\n"
      "    let colEl = document.getElementById(color_id);\n"
      "    if (colEl) colEl.value = evt.target.value;\n"
      "  }"
      "</script>";
  }
}
