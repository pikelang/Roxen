// $Id: module.pmod,v 1.5 2009/10/08 14:50:59 mast Exp $

#include "ldap_globals.h"

// To avoid 7.2 recursive resolver problems.
static program client_class;
#define client object
static object ldap_privates;
#undef  ASN1_CONTEXT_SEQUENCE
#define ASN1_CONTEXT_SEQUENCE		ldap_privates->asn1_context_sequence
#undef  ASN1_CONTEXT_BOOLEAN
#define ASN1_CONTEXT_BOOLEAN		ldap_privates->asn1_context_boolean
#undef  ASN1_CONTEXT_OCTET_STRING
#define ASN1_CONTEXT_OCTET_STRING	ldap_privates->asn1_context_octet_string
#undef  ASN1_CONTEXT_SET
...
#define EXCERPT(STR) (sizeof (STR) > 30 ? (STR)[..27] + "..." : (STR))
  ...
  if (!ldap_privates) ldap_privates = master()->resolv ("_NewLDAP.ldap_privates");
...
#define WS "%*[ \t\n\r]"
  ...
  function(void:string) read_filter_value = lambda ()
  ...
  function(void:object) make_filter_recur;
  make_filter_recur = lambda ()
  {
  ...
  read_filter_value = make_filter_recur = 0;
...
find_connection:
  ...
  if (!conn) {
    ...
    // 7.2 silliness for resolver lossage. :P
    if (!client_class) client_class = master()->resolv ("_NewLDAP.client");

    conn = client_class (parsed_url);
  }
