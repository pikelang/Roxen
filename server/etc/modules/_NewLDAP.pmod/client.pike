...
//
// $Id: client.pike,v 1.10 2007/10/08 15:58:30 mast Exp $
//
...
typedef string|Roxen.CharsetDecodeError|
  array(string|Roxen.CharsetDecodeError) ResultAttributeValue;
...
#define DECODE_DN(DN) do {						\
	if (mixed err = catch (DN = utf8_to_string (DN))) {		\
	  ...
	  if (flags & SEARCH_RETURN_DECODE_ERRORS)			\
	    DN = Roxen.CharsetDecodeError (DN, -1, 0, errmsg);		\
	  else								\
	    throw (Roxen.CharsetDecodeError (DN, -1, 0, errmsg));	\
	}								\
      } while (0)
...
#define DECODE_VALUE(ATTR, VALUE, DECODER) do {				\
	if (mixed err = catch {VALUE = DECODER (VALUE);}) {		\
	  ...
	  if (flags & SEARCH_RETURN_DECODE_ERRORS)			\
	    VALUE = Roxen.CharsetDecodeError (VALUE, -1, 0, errmsg);	\
	  else								\
	    throw (Roxen.CharsetDecodeError (VALUE, -1, 0, errmsg));	\
	}								\
      } while (0)
...
#define DECODE_VALUE(ATTR, VALUE, DECODER) do {				\
	...
	if (function(string:string) decoder =
	  ...
	  if (stringp (vals)) {
	    ...
	    foreach (vals; int i; string|Roxen.CharsetDecodeError val) {
...
#undef DECODE_VALUE
    ...
    object|int create(array rawres, void|int stuff, void|int flags_) {
    ...
    string|Roxen.CharsetDecodeError get_dn()
    {
...
#define PROFILE(STR, CODE...) do { CODE; } while(0)
  ...
  void create(string|mapping(string:mixed)|void url, object|void context)
  {
    ...
    info = ([ "code_revision" : ("$Revision: 1.10 $"/" ")[1] ]);
...
//!   value format wouldn't be reliable if they suddenly change that.
{
  ...
  if (!root_dse || zero_type (root_dse[attr])) {
	...
	foreach (indices (res), string attr)
	  ...
	  // they have not bothered to include in their schema. Send
...
{
  return _NewLDAP.make_filter (filter, ldap_version);
}
...
//!   @[Protocols.LDAP.make_filter].
{
  return _NewLDAP.get_cached_filter (filter, ldap_version);
}
...
//!   @[Protocols.LDAP.make_filter].
{
  ...
  result|int search (string|object|void filter, array(string)|void attrs,
    ...
    if (controls) {
      ...
      int i;
      foreach (indices (controls), string type) {
	array(int|string) data = controls[type];
	common_controls[i++] =
...
// it it's used for all other encountered terms.
{
  ...
  function(string:string) parse_qdstring =
    lambda (string what)
    {
      string qstr;
  ...
  function(string:string) parse_qdescr =
    lambda (string what)
    {
      string name;
  ...
  do {
    ...
    sscanf (str, "%[-;a-zA-Z0-9]%*[ ]%s", term_id, str);
    if (!sizeof (term_id))
  ...
  // Necessary in 7.2 to avoid garbage.
  parse_qdstring = parse_qdescr = 0;
