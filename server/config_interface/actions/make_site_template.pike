...
string parse(RequestID id)
{
  ...
  string res = "<h2 class='no-margin-top'>"+
    LOCALE(144, "Create site template")+"</h2>\n";

  // See [bug 5600] for a list of problems with this wizard.
  res += "<div class='notify error'>" + LOCALE(407, #"\
...
YMMV. This action is deprecated.") + "</div>\n";
  ...
  if (!conf) {
    ...
    if (conf_name) {
      res += sprintf("<div class='notify warn inline'>%s: %s</div><br>\n",
                     LOCALE(146, "Coundn't find configuration"),
                     conf_name);
    }
    res += sprintf("<p>%s</p>\n"
                   "<ul class='button-list'>%{<li><submit-gbutton2 name='conf-%s' width='200'>%s</submit-gbutton2></li>\n%}</ul>\n",
