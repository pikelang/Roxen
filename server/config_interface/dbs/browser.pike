...
mixed configure_ext_db_con( string db, RequestID id )
{
  ...
  return
    "<cf-title>"+_(414,"Configure external database connection")+"</cf-title>\n"
    +warning+
    "<table class='auto'>\n"
...
mapping|string parse( RequestID id )
{
  ...
  if(db && id->variables["run_q.x"])
  {
    ...
    foreach( (query/";\n")-({""}); int i; string q )
    {
      ...
      do {
        int qrows;
        qres += "<p>\n"
          "<table id='res' class='nice valign-top'><thead><tr>";
  ...
  res += ""
    "<table class='auto'><tr valign='center'>"
    "<td><span class='icon db x-large'></span></td>" +
    "<td>" + db_switcher( id ) + "</td></tr></table>\n"
    "<hr class='section'>"
    "<h3>Database <em>" + Roxen.html_encode_string (id->variables->db) + "</em></h3>\n";

  if (db_connect_error)
    res += "<div class='notify error'>" +
...
#define ADD_LI(X,Y) lis += ({ ([ "value" : (X), "bold" : Y ]) })
  ...
  if (string owner = format_table_owner (db_info)) {
    ADD_LI(sprintf((string)_(428,"Defined by %s."), owner), 0);
  ...
  if (db) {
    ...
    res += "<p><table id='tbls' class='nice db-list'>";
    ...
    string deep_table_info( string table )
    {
      ...
      string res = "<table id='tbl-details' class='tbl-details'>"
        "<thead>"
        "<tr>"
        "<th>Column</th>"
        "<th>Type</th>" +
    ...
    void add_table_info( string table, mapping tbi )
    {
      ...
      string res = "<tr id='tbl-" + Roxen.http_encode_url(table) + "'" +
        ...
        "#tbl-" + Roxen.http_encode_url(table) + "'"
        " class='icon table no-decoration'>"+
        table+"</a></td>"
        "<td class='num'>"+
	...
	if (tbl_info->inhibit_backups == "yes") {
	  res +=
            ...
            _(1142, "The table is not included in backups of this database.") +
	    "</div></div>";
	}
...
#define ADD_ACTION(X) if(!actions[X][2] || (actions[X][2] & flags) )    \
   res += sprintf("<link-gbutton href='%s?db=%s&amp;action=%s&amp;&usr.set-wiz-id;'>%s</link-gbutton>\n",\
                  id->not_query, id->variables->db, X, actions[X][0] )

#define ADD_ACTION2(X)                                                           \
  do {                                                                           \
    res += sprintf(                                                              \
      "<option value='%s?db=%s&amp;action=%s&amp;&usr.set-wiz-id;'>%s</option>", \
      id->not_query, id->variables->db, X, actions[X][0]);                       \
  } while (0)
