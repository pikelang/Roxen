...
constant magic_charset_variable_value = "åäö&#x829f;";
  ...
  //! See the ChiliMoon-automatic-charset-variable tag.
{
  ...
  if (has_prefix(åäö, "%") && !has_prefix(åäö, "%%")) {
    ...
    function decoder = get_client_charset_decoder(http_decode_string(åäö), id);
    if (decoder) {
...
class ScopeRequestHeader {
  ...
  string _sprintf(int t) { return "RXML.Scope(request-header)"; }
}
...
// This tag set always has the lowest priority.
{
  ...
  static void entities_prepare_context (RXML.Context c) {
    ...
    c->misc->scope_roxen=([]);
    c->add_scope("server",scope_roxen);
    c->misc->scope_page=([]);
